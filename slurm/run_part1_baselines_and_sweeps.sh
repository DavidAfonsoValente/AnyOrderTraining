...
# Baselines
J_SFT=$(sbatch   --parsable $DEP -C h100 slurm/train_standard_sft.sh)
J_IWM=$(sbatch   --parsable $DEP -C h100 slurm/train_prefix_sft_stage1.sh)
J_SFT2=$(sbatch  --parsable --dependency=afterok:${J_IWM} -C h100 \
                 slurm/train_prefix_sft_stage2.sh)

# AOMT-Mixed p_mask sweep
J_P015=$(sbatch  --parsable $DEP -C h100 slurm/train_aomt_mixed_p015.sh)
J_P025=$(sbatch  --parsable $DEP -C h100 slurm/train_aomt_mixed_p025.sh)
J_P040=$(sbatch  --parsable $DEP -C h100 slurm/train_aomt_mixed_p040.sh)
J_P050=$(sbatch  --parsable $DEP -C h100 slurm/train_aomt_mixed_p050.sh)

# Ablation A2: token-level masking
J_TOKEN=$(sbatch --parsable $DEP -C h100 slurm/train_aomt_mixed_token_level.sh)
