...
try:
    ...
    print(f'Note: Human trajectories download skipped or failed: {e}')
")
...
echo "if command -v module &> /dev/null; then" >> activate_env.sh
echo "    echo 'Attempting to load Python 3.11 module...'" >> activate_env.sh
echo "    module load python/3.11 || true" >> activate_env.sh
echo "fi" >> activate_env.sh
echo "# Set JAVA_HOME for pyserini (WebShop search engine)" >> activate_env.sh
echo "if [ -d \"\$HOME/miniconda3/envs/py311\" ]; then" >> activate_env.sh
echo "    export JAVA_HOME=\"\$HOME/miniconda3/envs/py311\"" >> activate_env.sh
echo "elif command -v conda &> /dev/null; then" >> activate_env.sh
echo "    export JAVA_HOME=\$(conda info --base)/envs/py311" >> activate_env.sh
echo "fi" >> activate_env.sh
echo "if [ -n \"\$JAVA_HOME\" ]; then export PATH=\$JAVA_HOME/bin:\$PATH; fi" >> activate_env.sh
