...
def main():
    ...
    device_name = f"cuda:{dist.get_rank() % torch.cuda.device_count()}" if dist.is_initialized() else "cuda:0"
    ...
    for epoch in range(config["train"]["num_epochs"]):
        ...
        for micro_step, batch in enumerate(pbar):
            ...
            with torch.amp.autocast("cuda", enabled=use_bf16, dtype=torch.bfloat16):
                outputs = model(input_ids=input_ids, attention_mask=attn_mask, use_cache=False, return_dict=True)
