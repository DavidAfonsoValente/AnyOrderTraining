...
from any_order_training.data import process_any_order_sft_example
...
@dataclass
class AnyOrderModelArguments(ModelArguments):
...
@dataclass
class AnyOrderDataArguments(DataArguments):
    ...
    training_mode: Literal["any_order", "causal", "prefix"] = field(
        default="any_order",
        metadata={"help": "Training mode."},
    )
    mask_prob: float = field(
        default=0.5,
        metadata={"help": "Masking probability for any_order training mode."},
    )
...
@dataclass
class Arguments:
    model: "AnyOrderModelArguments" = field(default_factory=AnyOrderModelArguments)
    data: "AnyOrderDataArguments" = field(default_factory=AnyOrderDataArguments)
    train: "AnyOrderTrainingArguments" = field(default_factory=AnyOrderTrainingArguments)
...
def main():
    ...
    if args.train.global_rank == 0 and args.train.save_hf_weights and save_checkpoint_path is not None:
        ...
        logger.info_rank0(f"Huggingface checkpoint saved at {hf_weights_gpath} successfully!")
